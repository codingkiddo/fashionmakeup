...
//chat
 ...
 function inscritptionAdmin($utilisateur,$confirmkey)
 {    
  $confirme=0;
  $role=1;
            $sql="insert into membre (pseudo,mail,motdepasse,confirmkey,confirme,role) values (:pseudo,:mail,:motdepasse,:confirmkey,:confirme,:role)";
        $db = config::getConnexion();
        try{
        $req=$db->prepare($sql);
